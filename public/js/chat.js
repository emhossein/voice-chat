...
var private_progress_bar = document.getElementById(
  "private-upload-progress-bar"
);
var private_progress_bar_inner = document.getElementById(
...
socket.on("notifyUser", function (data) {
  ...
  var private_message_button = document.getElementsByClassName(
...
socket.on("openChatWindow", function (user) {
  ...
  document
    .getElementsByClassName("private-message-send")[0]
...
$("#private-message-form").on("submit", function (e) {
  ...
  var private_userid = document.getElementsByClassName(
    "private-message-send"
  )[0].id;
...
socket.on("locationMessageSuccessful", function (message) {
  var formattedTime = moment(message.message.location.createdAt).format(
    "h:mm a"
  );
  //create an object of message
  var message_object = {
...
socket.on("incomingCall", function (user) {
  if (!calling) {
    ...
    if (id != user.id) {
      ...
      document.getElementsByClassName("alert-text")[0].innerHTML =
        user.name + " Calling......";
      caller_id = user.id;
    } else {
      ...
      $("#audio-call").attr("disabled", true);
      $("#video-call").attr("disabled", true);
      document.getElementById("audio-calling-text").innerHTML =
        user.name + " Calling......";
      modal_close_button[0].setAttribute("disabled", "disabled");
    }
...
socket.on("incomingVideoCall", function (user) {
  if (!calling) {
    ...
    if (id != user.id) {
      ...
      $("#alert-box").removeClass("alert-hide");
      document.getElementsByClassName("alert-text")[0].innerHTML =
        user.name + " Video Calling......";
      video_caller_id = user.id;
    } else {
      document.getElementById("video-calling-text").innerHTML =
        user.name + " Video Calling......";
      //appear the call receive section
      $("#video-call-receive-section").removeClass("accept-reject-video-call");
      $("#audio-call").attr("disabled", true);
